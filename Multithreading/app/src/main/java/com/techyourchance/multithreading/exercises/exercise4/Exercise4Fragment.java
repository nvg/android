...
public class Exercise4Fragment extends BaseFragment {
    ...
    private int mNumberOfThreads; // safe - read only after init on the main thread
    private ComputationRange[] mThreadsComputationRanges; // safe - read only after init
    private BigInteger[] mThreadsComputationResults;
    private final AtomicInteger mNumOfFinishedThreads = new AtomicInteger(0); // needs synchronization
    private volatile boolean mAbortComputation; // not safe - threads may be running after it's been changed - need volatile
    ...
    private void waitForThreadsResultsOrTimeoutOrAbort() {
        while (mNumOfFinishedThreads.get() != mNumberOfThreads
                && !mAbortComputation && !isTimedOut()) {
            synchronized (MONITOR) {
                try {
                    MONITOR.wait(getRemainingMillisToTimeout());
